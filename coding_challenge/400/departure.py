...
def heisenberg_trotter(couplings, p, time, depth):
    ...
    # Put your code here #
    for i in range(4):
        -1*couplings[3]*qml.PauliX(i)
        if i == 3:
            qml.IsingXX(2*time*couplings[0]/4, wires=[i,0])
            qml.IsingYY(2*time*couplings[1]/4, wires=[i,0])
            qml.IsingZZ(2*time*couplings[2]/4, wires=[i,0])
        else:
            qml.IsingXX(2*time*couplings[0]/4, wires=[i,i+1])
            qml.IsingYY(2*time*couplings[1]/4, wires=[i,i+1])
            qml.IsingZZ(2*time*couplings[2]/4, wires=[i,i+1])
    return qml.state()
